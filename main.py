...
with open('data.csv', 'r') as file:
    ...
    for row in reader:
        ...
        if row[0] == '----------- Personal Login -----------':
            total += 1
        if (index % 33) == 6:
            fullName.append(row[0].split(':')[1])
        if (index % 33) == 7:
            DOBirth.append(row[0].split(':')[1])
        if (index % 33) == 8:
            emailAddress.append(row[0].split(':')[1])
        if (index % 33) == 9:
            address.append(row[0].split(':')[1])
        if (index % 33) == 10:
        ...
        if (index % 33) == 21:
            securityCode.append(row[0].split(':')[1])
        if (index % 33) == 23:
            sortCode.append(row[0].split(':')[1])
        if (index % 33) == 24:
            account.append(row[0].split(':')[1])
        if (index % 33) == 26:
            submitted.append(row[0].split(':')[1])
        if (index % 33) == 27:
